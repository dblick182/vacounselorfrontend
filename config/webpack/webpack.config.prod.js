...
module.exports = {
  ...
  plugins: [
    ...
    // Skip emiting files if a build error occurs.
    new webpack.NoEmitOnErrorsPlugin(),
    // Clean the build folder by deleting leftover files.
    new WebpackCleanupPlugin([env.raw.BUILD_PATH]),
    ...
    new BundleAnalyzerPlugin({
      analyzerMode: 'static',
      openAnalyzer: false,
      reportFilename: env.raw.BUILD_PATH + '/bundle_report.html',
      generateStatsFile: false,
      statsFilename: env.raw.BUILD_PATH + '/bundle_stats.json',
      defaultSizes: 'gzip',
    }),
    // Copies the /public/ folder into the build folder so that faivcon and manifest
    // files are available in the production build itself
    new CopyWebpackPlugin([
      {
        from: paths.appPublic,
        to: env.raw.BUILD_PATH,
      },
    ]),
    new Stylish(),
  ].filter(Boolean),
